...
> [!question] 궁금한 점
> 어차피 이 뒤에서도 다 non-generate이라고 가정하던데 비퇴화 조건이 왜 중요한가? 만약 퇴화되면 어떤 문제가 생길까? (다음 챕터에서 나오려나)
...
이 사상은 내적이 맞을까? 예시로 다항함수 공간 $\mathbb{P}$의 부분공간으로 $V$를 잡아보자. 임의의 함수 $f = x$, $g = x^2$이라 하면 $\langle x, x^2 \rangle = \int^1_0 x \cdot x^2 dt=\int^1_0 x^2 \cdot x dt=1/4$. 일단 스칼라가 나오는 것은 맞고, 곱셈의 교환법칙에 따라 대칭성도 성립한다. 분배법칙에 의해 선형성도 자명하다. 즉 내적이다. 이렇게 두 개의 벡터를 넣어서 스칼라를 만드는 연산 중 대칭성과 선형성이 보장되는 연산을 스칼라 내적이라고 한다.
